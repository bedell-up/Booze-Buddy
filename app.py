...
@app.post("/lookup-barcode/", response_model=Dict)
async def lookup_barcode(
    ...
    """
    Look up a product by barcode using EANData API and optionally add it to inventory.
    """
    try:
        # EANData API configuration
        api_key = "C26AEA5E6812C236"
        eandata_url = f"https://api.eandata.com/product?v=2&find={barcode}&keycode={api_key}&mode=json"
        
        response = requests.get(eandata_url)
        ...
        # Check for successful response from EANData
        if data.get("status", {}).get("code") != 200:
            raise HTTPException(
                status_code=404, 
                detail=f"Barcode not found or API error: {data.get('status', {}).get('message', 'Unknown error')}"
            )
        ...
        # Try to determine the type of alcohol from the category
        alcohol_type = ""
        
        # Check category and product name for alcohol types
        alcohol_keywords = {
            ...
            "liqueur": ["liqueur", "cordial", "schnapps"],
            "wine": ["wine", "vino", "champagne"],
            "beer": ["beer", "ale", "lager", "stout"]
        }
        
        for alcohol, keywords in alcohol_keywords.items():
            if any(keyword in category.lower() for keyword in keywords):
                alcohol_type = alcohol
                break
        
        # If no type determined, try to guess from the product name
        if not alcohol_type:
            for alcohol, keywords in alcohol_keywords.items():
        ...
        # Still no type? Default to "spirit"
        if not alcohol_type:
            alcohol_type = "spirit"
